...
def main():
    ...
    sol  = sp.solve_ivp(ODE, (0, time), state_0, rtol = 1e-13) # solving the ODE 
    ...
    angular_vel_vecs = [] 
    h_t0 = np.linalg.cross(position_vecs[0], velocity_vecs[0])
    for vel_vec, pos_vec in zip(velocity_vecs, position_vecs): 
        h_t = np.linalg.cross(pos_vec,vel_vec) 
        angular_vel_vecs.append(np.linalg.norm(h_t - h_t0)/np.linalg.norm(h_t0))


    sol2  = sp.solve_ivp(ODE, (0, time), state_0, rtol = 1e-7) # solving the ODE 
    ...
    angular_vel_vecs2 = [] 
    h_t02 = np.linalg.cross(position_vecs2[0], velocity_vecs2[0])
    for vel_vec, pos_vec in zip(velocity_vecs2, position_vecs2): 
        h_t2 = np.linalg.cross(pos_vec,vel_vec) 
        angular_vel_vecs2.append(np.linalg.norm(h_t2 - h_t02)/np.linalg.norm(h_t02))
    ...
    fig.savefig("orbit_13")
    fig2.savefig("conv_13")
    fig3.savefig("orbit_7")
    fig4.savefig("conv_7")
    plt.show()
